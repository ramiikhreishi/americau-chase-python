...
def MySQL_User_Table():
    ...
    if school_name_pass == "":
        mycursor_HLGD.execute('SELECT Date, sum(Minutes) as "Minutes" FROM HLGD.HL3_Time_In_Game WHERE Date >= "'+sdate_pass+'" AND Date <= "'+edate_pass+'" group by Date;')
        result = mycursor_HLGD.fetchall()

    else:
        mycursor_HLGD.execute('SELECT Date, sum(Minutes) as "Minutes" FROM HLGD.HL3_Time_In_Game WHERE school_name LIKE "%'+school_name_pass+'%" AND Date >= "'+sdate_pass+'" AND Date <= "'+edate_pass+'" group by Date;')
        result = mycursor_HLGD.fetchall()
